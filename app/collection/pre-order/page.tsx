...
async function getPreOrderProducts(): Promise<StoreProduct[]> {
...
export const metadata: Metadata = {
  title: "Pre-Order",
  description: "Pre-order the latest Beyblade X releases before they sell out. Lock in your price today and get your Beyblade delivered on release day.",
  openGraph: {
    title: "Pre-Order - xzvl.store",
    description: "Pre-order the latest Beyblade X releases before they sell out. Lock in your price today and get your Beyblade delivered on release day.",
...
export default async function PreOrderCollectionPage() {
        ...
        <section className="max-w-[1440px] mx-auto px-4 md:px-16 py-12">
          {products.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {products.map((p) => (
                <ProductCard key={p.id} product={p} useSocialImage />
              ))}
            </div>
          )}
        </section>
